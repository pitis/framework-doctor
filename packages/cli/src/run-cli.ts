...
interface CliRuntime {
  ...
  runDoctor: (framework: Framework, doctorArguments: string[]) => number;
  wireWatchMode: (directoryPath: string, runScan: () => void) => void;
}
...
const parsePackageDependencies = (directoryPath: string): Record<string, string> | null => {
  const packagePath = path.join(directoryPath, 'package.json');
  if (!fs.existsSync(packagePath)) {
    return null;
  }
  try {
    const packageJson = JSON.parse(fs.readFileSync(packagePath, 'utf-8')) as Record<
      string,
      unknown
    >;
    return {
...
const detectFramework = (directoryPath: string): FrameworkDetectionResult => {
  const dependencyMap = parsePackageDependencies(directoryPath);
  if (!dependencyMap) {
...
const resolveDoctorCli = (doctorPackage: string): string => {
  const require = createRequire(import.meta.url);
  const indexPath = require.resolve(doctorPackage);
...
const printUnsupportedFrameworkValue = (frameworkValue: string, runtime: CliRuntime): void => {
...
const wireWatchMode = (directoryPath: string, runScan: () => void): void => {
...
export const main = (
  ...
  if (forcedFrameworkValue && !isFramework(forcedFrameworkValue)) {
    printUnsupportedFrameworkValue(forcedFrameworkValue, runtime);
    return 1;
  }

  const { directoryPath, doctorArguments, forcedFramework, shouldWatch } =
    parseCliArguments(rawArguments);
  ...
  if (!frameworkToRun) {
    printFrameworkDetectionError(directoryPath, detectionResult.matchedFrameworks, runtime);
    return 1;
  }

  if (!shouldWatch) {
    return runtime.runDoctor(frameworkToRun, doctorArguments);
  }

  const runScan = () => {
    runtime.runDoctor(frameworkToRun, doctorArguments);
  };

  runScan();
  runtime.log('\nWatching for changes... (Ctrl+C to stop)\n');
  runtime.wireWatchMode(directoryPath, runScan);
  return 0;
};
