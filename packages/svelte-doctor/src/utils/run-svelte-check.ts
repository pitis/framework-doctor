...
const parseMachineFormat = (rawOutput: string): SvelteCheckRecord[] => {
  ...
  for (const line of lines) {
    ...
    if (!trimmed || /^\d+\s+START\s/.test(trimmed) || /^\d+\s+COMPLETED\s/.test(trimmed) || /^\d+\s+FAILURE\s/.test(trimmed)) continue;

    const match = trimmed.match(/^\d+\s+(ERROR|WARNING)\s+"((?:[^"\\]|\\.)*)"\s+(\d+):(\d+)\s+"((?:[^"\\]|\\.)*)"$/);
    if (!match) continue;
