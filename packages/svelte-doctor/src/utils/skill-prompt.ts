...
const SKILL_BODY = `Scans your Svelte codebase for security, performance, correctness, and architecture issues. Outputs a 0-100 score with actionable diagnostics.
...
const AGENTS_CONTENT = `# Svelte Doctor
...
const CODEX_AGENT_CONFIG = `interface:
  ...
  short_description: "Diagnose and fix Svelte codebase health issues"
`;
...
const installSkill = (): void => {
  ...
  if (installedCount === 0) {
    ...
    logger.success('Done! The skill will activate when working on Svelte projects.');
  }
};
...
export const maybePromptSkillInstall = async (shouldSkipPrompts: boolean): Promise<void> => {
  ...
  logger.dim(
    `   Install the ${highlighter.info('svelte-doctor')} skill to teach Cursor, Claude Code,`,
  );
  logger.dim('   and other AI agents how to diagnose and fix Svelte issues.');
  logger.break();
