...
import { runVueTsc } from './utils/run-vue-tsc.js';
...
const formatFrameworkName = (framework: string): string => (framework === 'nuxt' ? 'Nuxt' : 'Vue');
...
const printProjectDetection = (
  ...
  completeStep(
    `Detecting Vue version. Found ${highlighter.info(`Vue ${projectInfo.vueVersion ?? 'unknown'}`)}.`,
  );
...
export const scan = async (
  ...
  const vueIncludePaths = computeVueIncludePaths(includePaths) ?? includePaths;
  ...
  const vueTscPromise = (async () => {
    ...
    try {
      ...
      vueTscSpinner?.fail('Vue type check failed (non-fatal, skipping).');
  ...
  const [vueTscDiagnostics, lintDiagnostics, deadCodeDiagnostics, securityDiagnostics] =
    await Promise.all([vueTscPromise, lintPromise, deadCodePromise, securityPromise]);

  const diagnostics = combineDiagnostics(
    [...vueTscDiagnostics, ...lintDiagnostics],
    deadCodeDiagnostics,
    securityDiagnostics,
    directory,
    isDiffMode,
