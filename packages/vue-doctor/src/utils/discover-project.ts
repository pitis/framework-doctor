...
import type { PackageJson, ProjectInfo, VueFramework, WorkspacePackage } from '../types.js';
...
const hasVueDependency = (packageJson: PackageJson): boolean => {
  ...
  return Object.keys(allDeps).some(
    (packageName) => packageName === 'vue' || packageName === 'nuxt',
  );
};
...
const detectFramework = (dependencies: Record<string, string>): VueFramework =>
  dependencies.nuxt ? 'nuxt' : 'vue';
...
export const discoverProject = (directory: string): ProjectInfo => {
  ...
  const vueVersion = dependencies.vue ?? null;
